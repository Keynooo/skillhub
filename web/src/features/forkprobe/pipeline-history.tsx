...
import { usePipelineHistory, usePipelineHistoryDetail } from './use-forkprobe-queries'
import { PipelineSummary } from './pipeline-summary'
import type { PipelineHistoryItem, PipelineStatusResponse } from './forkprobe-api'
...
/**
 * Modal listing the user's persisted pipeline (编排) runs, with drill-in to
 * view a past run's full stage results.
 */
export function PipelineHistory({
  ...
  const { data: history, isLoading } = usePipelineHistory(20, open)
  const [selectedItem, setSelectedItem] = useState<PipelineHistoryItem | null>(null)
  const { data: detail, isLoading: detailLoading } = usePipelineHistoryDetail(
    open ? (selectedItem?.pipelineId ?? null) : null,
  )
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h2 className="text-lg font-semibold" style={{ color: 'hsl(var(--foreground))' }}>
              {selectedItem ? t('forkprobe.pipelineHistoryDetail') : t('forkprobe.pipelineHistory')}
            </h2>
          </div>
          <button
            onClick={onClose}
...
function HistoryList({
  ...
  history: PipelineHistoryItem[] | undefined
  isLoading: boolean
  ...
  if (!history || history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-3">{t('forkprobe.noPipelineHistory')}</p>
      </div>
    )
  }
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                    ...
                    <span>{t('forkprobe.stageCount', { n: item.stageCount })}</span>
...
function HistoryDetail({
  ...
  detail: PipelineStatusResponse | undefined
  isLoading: boolean
}) {
  const { t, i18n } = useTranslation()
  if (isLoading) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
        {t('forkprobe.stageCount', { n: detail.lanes.reduce((s, l) => s + l.stages.length, 0) })}
      ...
      <PipelineSummary lanes={detail.lanes} />
    </div>
  )
}
