...
### D. 公司内部 open relay（内网免认证）

公司网络不允许直连外部 mail server，使用内部 open relay（免认证、明文 25）。

```dotenv
SPRING_MAIL_HOST=relay-b.asustek.com.cn
SPRING_MAIL_PORT=25
SPRING_MAIL_USERNAME=
SPRING_MAIL_PASSWORD=
SPRING_MAIL_SMTP_AUTH=false
...
SKILLHUB_AUTH_PASSWORD_RESET_CODE_EXPIRY=PT10M
SKILLHUB_AUTH_PASSWORD_RESET_FROM_ADDRESS=Cheng_Hu@asus.com
SKILLHUB_AUTH_PASSWORD_RESET_FROM_NAME=SkillHub
```
